...
self.addEventListener('push', e => {
    ...
    e.waitUntil(
        self.registration.showNotification(data.title || 'VibeStore 🛍️', options)
    );
});
...
// Fetch: network first, cache fallback for the app shell
self.addEventListener('fetch', e => {
    if (e.request.method !== 'GET') return;
    // Only cache same-origin requests (not Firebase/Metered/CDN)
